...
import type { Summary } from "@/games/reflexrush/engine";
...
export const Route = createFileRoute("/reflex-rush/challenge/$code")({
  head: () => ({
    meta: [
      { title: "Reflex Rush Challenge — Beat This Score" },
      {
        name: "description",
        content:
          "Open a Reflex Rush challenge link, tap the exact same targets and climb this challenge's private leaderboard. No account needed.",
      },
      { property: "og:title", content: "Can you beat this Reflex Rush score?" },
      {
        property: "og:description",
        content: "Same targets, same 30 seconds. Enter your name and play.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: ReflexRushChallengePage,
});

function ReflexRushChallengePage() {
  ...
  const att = useChallengeAttempt("reflexrush", challenge?.id ?? null, meKey);
    ...
    return (
      <GameShell>
        <ReflexArena
          ...
          note={summary ? undefined : "Challenge targets — same for everyone"}
          onAction={att.record}
  ...
  return (
    <ChallengeIntro
      ...
      subtitle="Same targets, same 30 seconds. One attempt only."
      onPlay={() => {
        if (played) return;
