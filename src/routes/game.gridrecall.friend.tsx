...
export const Route = createFileRoute("/game/gridrecall/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Grid Recall With a Friend — Private Room" },
      {
        name: "description",
        content:
          "Create a private Grid Recall room, share the code and take on the same patterns as your friend.",
      },
      { property: "og:title", content: "Private Grid Recall Room" },
      { property: "og:description", content: "Share a room code and play your friend." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: GridRecallFriend,
});

function GridRecallFriend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/gridrecall" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Grid Recall" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
