...
export const Route = createFileRoute("/game/battlexo/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Battle XO With a Friend — Private Room" },
      {
        name: "description",
        content: "Create a private Battle XO room and duel your friend in realtime.",
      },
      { property: "og:title", content: "Private Battle XO Room" },
      { property: "og:description", content: "Share a room code and play instantly." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: FriendBattle,
});
...
function FriendBattle() {
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Battle XO" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
