...
export const Route = createFileRoute("/game/ghostxo/online")({
  head: () => ({
    meta: [
      { title: "Ghost XO Online — Ranked Realtime Matches" },
      {
        name: "description",
        content:
          "Get matched with a random Ghost XO opponent for realtime ranked duels with turn timers and best-of series.",
      },
      { property: "og:title", content: "Ghost XO Online" },
      { property: "og:description", content: "Ranked realtime matchmaking with live rating." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: OnlineGhost,
});

function OnlineGhost() {
  ...
  return (
    <GameShell>
      <MatchSearch
        room={room}
        onBotFallback={() => void navigate({ to: "/game/ghostxo/bot" })}
        onExit={() => void navigate({ to: "/game/ghostxo" })}
      />
    </GameShell>
  );
}
