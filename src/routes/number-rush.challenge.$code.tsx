...
import type { Summary } from "@/games/numberrush/engine";
...
export const Route = createFileRoute("/number-rush/challenge/$code")({
  head: () => ({
    meta: [
      { title: "Number Rush Challenge — Beat This Score" },
      {
        name: "description",
        content:
          "Open a Number Rush challenge link, solve the exact same questions and climb this challenge's private leaderboard. No account needed.",
      },
      { property: "og:title", content: "Can you beat this Number Rush score?" },
      {
        property: "og:description",
        content: "Same questions, same 30 seconds. Enter your name and play.",
      },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: NumberRushChallengePage,
});

function NumberRushChallengePage() {
  ...
  const att = useChallengeAttempt("numberrush", challenge?.id ?? null, meKey);
    ...
    return (
      <GameShell>
        <NumberArena
          ...
          seed={challenge.seed}
          duration={att.playSeconds(30)}
          meName={name.trim() || "You"}
          note={summary ? undefined : "Challenge questions — same for everyone"}
          onAction={att.record}
  ...
  return (
    <ChallengeIntro
      ...
      subtitle="Same questions, same 30 seconds. One attempt only."
      onPlay={() => {
        if (played) return;
