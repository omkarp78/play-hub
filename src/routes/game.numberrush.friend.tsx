...
export const Route = createFileRoute("/game/numberrush/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Number Rush With a Friend — Private Room" },
      {
        name: "description",
        content:
          "Create a private Number Rush room, share the code and race your friend through the same questions.",
      },
      { property: "og:title", content: "Private Number Rush Room" },
      { property: "og:description", content: "Share a room code and race your friend." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: NumberRushFriend,
});

function NumberRushFriend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/numberrush" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Number Rush" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
