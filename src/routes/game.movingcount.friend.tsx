...
export const Route = createFileRoute("/game/movingcount/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Moving Count With a Friend — Private Room" },
      {
        name: "description",
        content:
          "Create a private Moving Count room, share the code and race your friend through the same cards.",
      },
      { property: "og:title", content: "Private Moving Count Room" },
      { property: "og:description", content: "Share a room code and race your friend." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: MovingCountFriend,
});

function MovingCountFriend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/movingcount" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Moving Count" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
