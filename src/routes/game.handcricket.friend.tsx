...
export const Route = createFileRoute("/game/handcricket/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Hand Cricket With a Friend — Private Room" },
      {
        name: "description",
        content: "Create a private Hand Cricket room and play your friend in realtime.",
      },
      { property: "og:title", content: "Private Hand Cricket Room" },
      { property: "og:description", content: "Share a room code and play instantly." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: CricketFriend,
});

function CricketFriend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/handcricket" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Hand Cricket" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
