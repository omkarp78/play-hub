...
import {
  ...
  type BotLevel,
  type Summary,
} from "@/games/gridrecall/engine";
import { usePlayer } from "@/hooks/useAuth";
import { useGuestStore } from "@/lib/identity";
import { recordResult } from "@/lib/results";
import { sfx } from "@/lib/sound";

export const Route = createFileRoute("/game/gridrecall/bot")({
  head: () => ({
    meta: [
      { title: "Grid Recall vs Bot — Practice Your Memory" },
      {
        name: "description",
        content:
          "Race a bot through the Grid Recall board and sharpen your recall. Casual only — no rating change.",
      },
      { property: "og:title", content: "Grid Recall vs Bot" },
      { property: "og:description", content: "Match pairs faster than the bot." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: GridRecallBot,
});
