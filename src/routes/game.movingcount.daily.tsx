...
import { dailySeed, type Summary } from "@/games/movingcount/engine";
...
export const Route = createFileRoute("/game/movingcount/daily")({
  head: () => ({
    meta: [
      { title: "Moving Count Daily Challenge — Same Cards For Everyone" },
      {
        name: "description",
        content:
          "One attempt a day on the exact same Moving Count board. No account needed — enter a name and climb today's leaderboard.",
      },
      { property: "og:title", content: "Moving Count Daily Challenge" },
      { property: "og:description", content: "One attempt. One board. Global ranks." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: MovingCountDaily,
});

function MovingCountDaily() {
  const player = usePlayer();
  const daily = useDailyChallenge("movingcount");
    ...
    return (
      <GameShell>
        <MovingCountArena
          ...
          note={summary ? undefined : "Daily cards — same for everyone"}
  ...
  return (
    <AppShell>
      <PageHeader title="Daily Challenge" back="/game/movingcount" />
      ...
      <div className="rounded-3xl bg-card p-6 text-center soft-card">
        ...
        <h2 className="mt-4 font-display text-xl font-bold">Today&apos;s cards</h2>
