...
export const Route = createFileRoute("/game/reflexrush/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "Reflex Rush With a Friend — Private Room" },
      {
        name: "description",
        content:
          "Create a private Reflex Rush room, share the code and race your friend through the same targets.",
      },
      { property: "og:title", content: "Private Reflex Rush Room" },
      { property: "og:description", content: "Share a room code and race your friend." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: ReflexRushFriend,
});

function ReflexRushFriend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/reflexrush" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="Reflex Rush" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
