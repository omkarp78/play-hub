...
export const Route = createFileRoute("/game/rush24/friend")({
  validateSearch: (search: Record<string, unknown>): Search => ({
    code: typeof search["code"] === "string" ? search["code"] : undefined,
  }),
  head: () => ({
    meta: [
      { title: "24 Rush With a Friend — Private Room" },
      {
        name: "description",
        content:
          "Create a private 24 Rush room, share the code and race your friend through the same ten puzzles.",
      },
      { property: "og:title", content: "Private 24 Rush Room" },
      { property: "og:description", content: "Share a room code and play your friend." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: Rush24Friend,
});

function Rush24Friend() {
  ...
  if (!player.id) {
    return (
      <AppShell>
        <PageHeader title="Play With a Friend" back="/game/rush24" />
        <GuestGate />
      </AppShell>
    );
  }
  ...
  if (joined && !joined.guest_id && room.isHost) {
    return (
      <GameShell>
        <RoomInvite code={joined.code} gameName="24 Rush" onLeave={() => void room.leave()} />
      </GameShell>
    );
  }
