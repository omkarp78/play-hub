...
import { dailySeed, type Summary } from "@/games/gridrecall/engine";
...
export const Route = createFileRoute("/game/gridrecall/daily")({
  head: () => ({
    meta: [
      { title: "Grid Recall Daily Challenge — Same Patterns For Everyone" },
      {
        name: "description",
        content:
          "One attempt a day on the exact same Grid Recall patterns. No account needed — enter a name and climb today's leaderboard.",
      },
      { property: "og:title", content: "Grid Recall Daily Challenge" },
      { property: "og:description", content: "One attempt. One pattern set. Global ranks." },
      { property: "og:type", content: "website" },
      { name: "twitter:card", content: "summary_large_image" },
    ],
  }),
  component: GridRecallDaily,
});

function GridRecallDaily() {
  const player = usePlayer();
  const daily = useDailyChallenge("gridrecall");
    ...
    return (
      <GameShell>
        <GridRecallArena
          ...
          note={summary ? undefined : "Daily patterns — same for everyone"}
  ...
  return (
    <AppShell>
      <PageHeader title="Daily Challenge" back="/game/gridrecall" />
      ...
      <div className="rounded-3xl bg-card p-6 text-center soft-card">
        ...
        <h2 className="mt-4 font-display text-xl font-bold">Today&apos;s patterns</h2>
        <p className="mt-1 text-sm text-muted-foreground">
          Everyone plays the exact same board today.
        </p>
